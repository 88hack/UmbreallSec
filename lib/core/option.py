...
from lib.core.Traceability import *
...
def main(path):
    ...
    group = optparse.OptionGroup(parser, "Mode", "UbSec running mode options")
    ...
    group.add_option("--srp", dest="logdir", default=False, action='store_true', help=u"打包当前系统的所有安全日志（暂不支持）")
    parser.add_option_group(group)
    ...
    # 扫描日志目录
    set_value('LOG_PATH', path + "/srp/UbSec.md")
    # 结果记录目录
    set_value('DB_PATH', path + "/sr/sr.md")
    # 扫描结果
    ...
    if options.logdir:
        ...
        mkfile()
        file_write(u'# 扫描报告\n')
        print(u'\033[1;32m# 开始扫描当前系统安全状态...\033[0m')
        # 获取恶意特征信息
