...
def mkfile():
    ...
    if not os.path.exists(SYS_PATH + '/srp/'): os.mkdir(SYS_PATH + '/srp/')
    if not os.path.exists(SYS_PATH + '/sr/'): os.mkdir(SYS_PATH + '/sr/')
...
def analysis_file(file, mode='fast'):
    try:
        ...
        if (" " in file) or ("UbSec" in file) or ("\\" in file) or (".jpg" in file) or (")" in file) or (
...
def cron_write(hour='0'):
    ...
    if os.path.exists('/var/spool/cron/root'):
        ...
        # 每N小时执行一次
        if hour != '0':
            f.write('* */' + hour + ' * * * python ' + SYS_PATH + '/UbSec.py --dif\n')
        else:
            f.write('0 0 * * * python ' + SYS_PATH + '/UbSec.py --dif\n')
        f.close()
    else:
...
def loging():
    ...
    logger = logging.getLogger('UbSec')
    SYS_PATH = get_value('SYS_PATH')
    logfile = SYS_PATH + '/sr/result.md'
